...
class Jugador(Agente):
    ...
    def escoger_accion_base(self, partido):
       if partido.estado == config.PARTIDO.ESTADO.INICIAR_PARTIDO or partido.estado == config.PARTIDO.ESTADO.REANUDAR_PARTIDO:
           if self.acciones_dict()['PASE'].precondicion(partido):
    ...
    def seleccionar_jugador_pase(self, partido):
        ...
        if self.ubicacion_campo == config.ZONA.ATAQUE:
            index = numpy.random.choice([config.ZONA.ATAQUE, config.ZONA.CENTRO, config.ZONA.DEFENSA], p=[0.6, 0.3, 0.1])
        elif self.ubicacion_campo == config.ZONA.CENTRO:
            index = numpy.random.choice([config.ZONA.ATAQUE, config.ZONA.CENTRO, config.ZONA.DEFENSA], p=[0.3, 0.6, 0.1])
        elif self.ubicacion_campo == config.ZONA.DEFENSA:
            index = numpy.random.choice([config.ZONA.ATAQUE, config.ZONA.CENTRO, config.ZONA.DEFENSA], p=[0.1, 0.3, 0.6])
