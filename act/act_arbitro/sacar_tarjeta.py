    ...
    def ejecutar(self, partido):
        ...
        if color:
            ...
            if partido.ultima_accion.agente.cantidad_tarjetas == 1:
                print(f'{partido.obtener_tiempo()} {self.descripcion()} {Fore.YELLOW} {self.estado} {Style.RESET_ALL} y le {self.sub_estado} y {config.ACCIONES.ESTADO.SACAR_TARJETA.MUESTRA_ROJA} a {partido.ultima_accion.agente.nombre}')
            else:
