...
class Escoger_alineacion(Accion):
    def __init__(self, agente) -> None:
        self.agente = agente
        self.tipo = config.ACT_ESCOGER_ALINEACION
    ...
    def precondicion(self, partido) -> bool:
        return partido.estado == config.INICIAR_PARTIDO
    ...
    def ejecutar(self, partido):
        ...
        escoger_esquema = config.ESQUEMA_DE_JUEGO[0].copy()
        posicion_jugadores = clasificar_jugadores(self.agente.equipo.jugadores)

        # SELECCIONAR JUGADORES EN CAMPO
        jugadores_campo = []
        for i in range(len(config.POSICIONES)):
            cant = escoger_esquema[i]
            while cant > 0:
                j = random.randint(0, len(posicion_jugadores[config.POSICIONES[i]]) - 1)
                jugadores_campo.append(posicion_jugadores[config.POSICIONES[i]].pop(j))
                cant -= 1
        ...
        for i in range(config.CANT_JUGADORES_BANCA):
